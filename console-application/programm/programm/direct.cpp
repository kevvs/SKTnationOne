...
void C_Cube::init(){
	...
	mes = hx * hy * hz;
	barycenter = C_Point(nodes[0] -> x + hx / 2.0, 
                       nodes[0] -> y + hy / 2.0, 
                       nodes[0] -> z + hz / 2.0);
  ...
  for(int i = 0; i < 3; i++){
    for(int j = 0; j < 3; j++){
      for(int k = 0; k < 3; k++){
        ...
        gauss_weights[(i * 3 + j) * 3 + k] = gauss_weights_1d[i] * gauss_weights_1d[j] * gauss_weights_1d[k];
        gauss_points[(i * 3 + j) * 3 + k] = C_Point(gauss_points_1d[i], gauss_points_1d[j], gauss_points_1d[k]);
      }
    }
  }
	jacobian = mes / 8.0;
...
C_Vector3 C_Cube::get_B(C_Point x) const{

	C_Vector3 result(0.0, 0.0, 0.0);

	for(size_t i = 0; i < 27; i++){
		
    double dx = x.x - gauss_points[i].x; 
    double dy = x.y - gauss_points[i].y; 
    double dz = x.z - gauss_points[i].z;
    double r = sqrt(dx * dx + dy * dy + dz * dz);
    double r2 = 1.0 / (r * r);
		double bla = jacobian * gauss_weights[i] / (4.0 * M_PI * r * r * r);
    ...
    double bz = bla * (p.x * (3.0 * dx * dz * r2) + p.y * (3.0 * dy * dz * r2) + p.z * (3.0 * dz * dz * r2 - 1.0));
		result = result + C_Vector3(bx, by, bz);
	}
	return result;
}

bool C_Include::inside(const C_Cube &p) const{
...
istream &operator >> (istream & is, C_Include &a){

	is >> a.x_min >> a.x_max >> a.y_min >> a.y_max >> a.z_min >> a.z_max >> a.p.x >> a.p.y >> a.p.z;
	return is;
}

C_Vector3 C_Area::get_B(C_Point x) const{
...
double C_Area::get_abs_B(C_Point x) const{
	return get_B(x).norm();
}

void C_Area::generate(ifstream &ifs){
	...
	size_t inc_num;
	ifs >> inc_num;
	vector<C_Include> includes;
	includes.resize(inc_num);
  for(size_t i = 0; i != inc_num; i++) {
    ifs >> includes[i];
  }  
	double hx = (x_max - x_min) / x_num;
	double hy = (y_max - y_min) / y_num;
	double hz = (z_max - z_min) / z_num;
	nodes.resize((x_num + 1) * (y_num + 1) * (z_num + 1));
	for(size_t i = 0, m = 0; i <= x_num; i++){
		double x_curr = x_min + hx * (double)i;
		
    for(size_t j = 0; j <= y_num; j++){
			double y_curr = y_min + hy * (double)j;
			
      for(size_t k = 0; k <= z_num; k++){
				double z_curr = z_min + hz * (double)k;
				nodes[m].x = x_curr;
				nodes[m].y = y_curr;
				nodes[m].z = z_curr;
				m++;
